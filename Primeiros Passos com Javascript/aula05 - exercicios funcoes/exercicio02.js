/*

2) Elabore um algoritmo que calcule o que deve ser pago por um produto, considerando o preço normal de etiqueta e a escolha da condição de pagamento.
Utilize os códigos da tabela a seguir para ler qual a condição de pagamento escolhida e efetuar o cálculo adequado.

Código Condição de pagamento:
1 - À vista Débito, recebe 10% de desconto;
2 - À vista no Dinheiro ou PIX, recebe 15% de desconto;
3 - Em duas vezes, preço normal de etiqueta sem juros;
4 - Acima de duas vezes, preço normal de etiqueta mais juros de 10%;

Fazendo usando funções dessa vez
*/

function aplicarDesconto(valor, desconto) {
    return valor - (valor * (desconto / 100));
}

function aplicarJuros(valor, juros) {
    return valor + (valor * (juros / 100));
}

function calcularPrecoFinal(precoEtiqueta, tipoPagamento) {
    if (tipoPagamento === 1) {
        return aplicarDesconto(precoEtiqueta, 10);
    } else if (tipoPagamento === 2) {
        return aplicarDesconto(precoEtiqueta, 15)
    } else if (tipoPagamento === 3) {
        return precoEtiqueta;
    } else if (tipoPagamento === 4) {
        return aplicarJuros(precoEtiqueta, 10);
    }
}

function imprimirPrecoFinal(precoEtiqueta, tipoPagamento) {
    console.log("O preço final é: R$" + calcularPrecoFinal(precoEtiqueta, tipoPagamento).toFixed(2));
}

(function () {
    const precoEtiqueta = 100;
    const tipoPagamento = 4;
    imprimirPrecoFinal(precoEtiqueta, tipoPagamento);
})();